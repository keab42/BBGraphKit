...
@property (nonatomic, strong) NSMutableDictionary *clippingLayers;

@end

@implementation BBLineGraph

- (NSMutableDictionary *)clippingLayers
{
    if (!_clippingLayers)
    {
        _clippingLayers = [NSMutableDictionary dictionary];
    }
    return _clippingLayers;
}
...
- (void)drawSeries:(NSUInteger)series {
    BOOL shouldDrawNegativeValues = YES;
    if ([self.delegate respondsToSelector:@selector(lineGraph:shouldPlotNegativeValuesForSeries:)])
    {
        shouldDrawNegativeValues = [self.delegate lineGraph:self shouldPlotNegativeValuesForSeries:series];
    }
    ...
    CAShapeLayer *clippingLayer = [self.clippingLayers objectForKey:@(series)];
    ...
    if (!seriesLayer) {
        ...
        if (shouldDrawNegativeValues)
        {
            [self.screenSpaceView.layer addSublayer:seriesLayer];
        }
        else
        {
            clippingLayer = [CALayer layer];
            
            [clippingLayer addSublayer:seriesLayer];
            clippingLayer.masksToBounds = YES;
            [self.screenSpaceView.layer addSublayer:clippingLayer];
            [self.clippingLayers setObject:clippingLayer forKey:@(series)];
        }
    ...
    if (clippingLayer)
    {
        CGRect clippingFrame = seriesLayer.frame;
        CGPoint point = CGPointMake(0, 0);
        if (self.scaleYAxisToValues)
            point.y -= self.lowestYValue;
        clippingFrame.size.height = [self convertPointToScreenSpace:point].y;
        clippingLayer.frame = clippingFrame;
    }
